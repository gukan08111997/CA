...
const jobRolesCA = [
  ...
  const btnCAPaperIntermediate = document.getElementById("ca-paper-intermediate");
  const btnCAEntryIntermediate = document.getElementById("ca-entry-intermediate");
  const btnCAJobIntermediate = document.getElementById("ca-job-intermediate");
  const btnCAInterIntermediate = document.getElementById("ca-inter-intermediate");
  const tabContent = document.querySelector(".tab-content");
  const list = document.createElement("ul");
  
  tabContent.append(list);
  function btnHighlighted(btn) {
    btnCAPaperIntermediate.className = "";
    btnCAEntryIntermediate.className = "";
    btnCAJobIntermediate.className = "";
    btnCAInterIntermediate.className = "";
  ...
  function handleClick(e) {
    btnHighlighted(e.target);
     if (e.target.id === "ca-paper-intermediate") {
      displayContent(papersCA[1]);
    } else if (e.target.id === "ca-entry-intermediate") {
      displayContent(entryRoutesCA[0]);
    } else if (e.target.id === "ca-job-intermediate") {
      displayContent(jobRolesCA[0]);
    } else if (e.target.id === "ca-inter-intermediate") {
      displayContent(jobRolesCA[1]);
    }
  }
  
  btnCAPaperIntermediate.addEventListener("click", handleClick);
  btnCAEntryIntermediate.addEventListener("click", handleClick);
  btnCAJobIntermediate.addEventListener("click", handleClick);
  btnCAInterIntermediate.addEventListener("click", handleClick);
