...
const jobRolesCA = [
  ...
  const btnCMAPaperFinal = document.getElementById("cma-paper-final");
  const btnCMAEntryFinal = document.getElementById("cma-entry-final");
  const btnCMAJobFinal = document.getElementById("cma-job-final");
  const btnCMAInterFinal = document.getElementById("cma-inter-final");
  const tabContent = document.querySelector(".tab-content");
  const list = document.createElement("ul");
  
  tabContent.append(list);
  function btnHighlighted(btn) {
    btnCMAPaperFinal.className = "";
    btnCMAEntryFinal.className = "";
    btnCMAJobFinal.className = "";
    btnCMAInterFinal.className = "";
  ...
  function handleClick(e) {
    btnHighlighted(e.target);
     if (e.target.id === "cma-paper-final") {
      displayContent(papersCMA[2]);
    } else if (e.target.id === "cma-entry-final") {
      displayContent(eligibilityCMA[0]);
    } else if (e.target.id === "cma-job-final") {
      displayContent(jobRolesCMA[0]);
    } else if (e.target.id === "cma-inter-final") {
      displayContent(jobRolesCMA[1]);
    }
  }
  
  btnCMAPaperFinal.addEventListener("click", handleClick);
  btnCMAEntryFinal.addEventListener("click", handleClick);
  btnCMAJobFinal.addEventListener("click", handleClick);
  btnCMAInterFinal.addEventListener("click", handleClick);
