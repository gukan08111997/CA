...
const jobRolesCA = [
  ...
  const btnCAPaperFinal = document.getElementById("ca-paper-final");
  const btnCAEntryFinal = document.getElementById("ca-entry-final");
  const btnCAJobFinal = document.getElementById("ca-job-final");
  const btnCAInterFinal = document.getElementById("ca-inter-final");
  const tabContent = document.querySelector(".tab-content");
  const list = document.createElement("ul");
  
  tabContent.append(list);
  function btnHighlighted(btn) {
    btnCAPaperFinal.className = "";
    btnCAEntryFinal.className = "";
    btnCAJobFinal.className = "";
    btnCAInterFinal.className = "";
  ...
  function handleClick(e) {
    btnHighlighted(e.target);
     if (e.target.id === "ca-paper-final") {
      displayContent(papersCA[2]);
    } else if (e.target.id === "ca-entry-final") {
      displayContent(entryRoutesCA[0]);
    } else if (e.target.id === "ca-job-final") {
      displayContent(jobRolesCA[0]);
    } else if (e.target.id === "ca-inter-final") {
      displayContent(jobRolesCA[1]);
    }
  }
  
  btnCAPaperFinal.addEventListener("click", handleClick);
  btnCAEntryFinal.addEventListener("click", handleClick);
  btnCAJobFinal.addEventListener("click", handleClick);
  btnCAInterFinal.addEventListener("click", handleClick);
